...
import 'device_id_setup_page.dart';
import 'device_name_setup_page.dart';
...
class _AddDevicePageState extends State<AddDevicePage> {
  ...
  double? _waterVolume = 20.0; // Default water volume in liters
  ...
  // Default sensors matching backend
  ...
  void _previousStep() {
    setState(() {
      switch (_currentStep) {
        case DeviceSetupStep.deviceId:
          // Already at first step, navigate back
  ...
  // Handle device ID selection from DeviceIdSetupPage
  void _handleDeviceIdSelected(String deviceId) {
    setState(() {
      _deviceId = deviceId;
      // Auto-generate device name based on device ID
      _deviceName = "Device $deviceId";
      _nextStep();
    });
  }
  
  // Handle device name selection from DeviceNameSetupPage
  ...
  // Handle water volume selection from WaterVolumeSetupPage
  ...
  // Show the completion dialog with animation
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                // Message with animation - fixed using FutureBuilder
                FutureBuilder(
                ...
                // Button with animation - fixed using FutureBuilder
                FutureBuilder(
  ...
  Future<void> _submitDeviceSetup() async {
    ...
    // Create device data structure matching backend expectations
    final deviceData = {
      "device_id": _deviceId,
      "user_id": widget.userId,
      "device_name": _deviceName,
      "type": "Arduino", // Default type
      "kit": "standard", // Default kit
      "emergency_stop": false,
      "auto_dose_enabled": false, // Default to disabled
      "actuators": defaultActuators,
      "sensors": defaultSensors,
      "actuator_conditions": [], // Initialize with empty conditions
      "wifi_ssid": _wifiSsid, // Add Wi-Fi SSID
      "wifi_password": _wifiPassword, // Add Wi-Fi password
      "water_volume_liters": _waterVolume, // Use the selected water volume
    };
  ...
  // Build the step progress indicator at the top of the screen
